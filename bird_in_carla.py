...
my_img = np.zeros(shape=(280,420,3),dtype=np.uint8)
my_img = None
...
class lane_keeping_module(object) :
    def __init__(self):
            ...
            # Create Trackbar
            cv2.createTrackbar("[be]top", "TrackBar", birdeye_top, 280, self.onChange)
            cv2.createTrackbar("[be]bottom", "TrackBar",birdeye_bottom, 280, self.onChange)
            cv2.createTrackbar("[be]top_width", "TrackBar",birdeye_top_width, 420, self.onChange)
            cv2.createTrackbar("[be]bottom_width", "TrackBar",birdeye_bottom_width, 420, self.onChange)
            cv2.createTrackbar("[be]birdeye_width", "TrackBar",birdeye_width,420, self.onChange)        
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            lane_keeping_module.image_callback(my_img)
    finally:
